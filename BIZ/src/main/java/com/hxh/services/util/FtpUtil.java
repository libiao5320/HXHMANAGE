...
public class FtpUtil {
     ...
     * @return boolean
     */
    public static boolean  uploadDiserPhoto(String fileName, InputStream inputStream)throws Exception {
        FTPClient ftp = getConnectionFTP(pu.getValue("hostName"),Integer.parseInt(pu.getValue("port")), pu.getValue("userName"), pu.getValue("passWord"));
        ftp.setFileType(FTP.BINARY_FILE_TYPE);
        ftp.changeWorkingDirectory(pu.getValue("disPhoto"));
        boolean rtnFlag = ftp.storeFile(fileName, inputStream);
        inputStream.close();
        return rtnFlag;
    }
    /**
...
//            downSign = true;
    ...
    public static boolean delDiserPhoto(String fileName) throws Exception{
        FTPClient ftp = getConnectionFTP(pu.getValue("hostName"),Integer.parseInt(pu.getValue("port")), pu.getValue("userName"), pu.getValue("passWord"));
        boolean downSign = false;
        ftp.changeWorkingDirectory(pu.getValue("disPhoto"));
        try {
